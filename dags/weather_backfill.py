...
def generate_backfill_dates(**context):
    ...
    logger.info(f"Generated {len(dates)} dates for backfill: {start_date_str} to {end_date_str}")
...
def fetch_historical_weather(lat, lon, timestamp):
    ...
    if response.status_code == 401:
        raise ValueError("Invalid API key or subscription required for One Call API 3.0")
...
def backfill_location_date(location, target_date_str):
    ...
    try:
        ...
        return {'status': 'failed', 'city': city, 'date': target_date_str, 'error': str(e)}
...
def execute_backfill(**context):
    ...
    logger.info(f"Starting backfill: {len(locations)} locations × {len(dates)} dates = {total_operations} operations")
